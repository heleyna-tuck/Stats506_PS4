...
```{r}
get_temp <- function(month, year_num, data, celsius = FALSE, average_fn = mean){
  # Validate and sanitize the input
  if (!is.numeric(year_num) || !year_num %in% data$year) {
    stop("Invalid year. Please provide a numeric year between 1997 and 2000.")
  }
  month <- month %>% tolower()
  if (is.numeric(month) && month %in% 1:12) {
    month <- month.name[month]
  } 
  else if (!is.character(month) || !month %in% substr(tolower(month.name), 1, 3)) {
  ...
  if(is.character(month) && tolower(month) %in% substr(tolower(month.name), 1, 3)){
    month <- as.numeric(match(tolower(month), substr(tolower(month.name), 1, 3)))
  }
print(typeof(year_num))
print(typeof(month))
print(year_num)
print(month)
print(head(data))
 # Filter the data for the specified year and month
  result <- data %>%
    filter(year == year_num, month_numeric == month) 
  print(result)
...
```{r}
result <- nnmaps %>%
  filter(year == 1999, month_numeric == 4)
```
