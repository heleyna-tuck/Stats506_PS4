...
get_temp <- function(month, year, data, celsius = FALSE, average_fn = mean()){
  ...
  if (!is.numeric(year) || !year %in% data$year) {
    stop("Invalid year. Please provide a numeric year between 2001 and 2020.")
  }
  
  if (is.numeric(month) && month %in% 1:12) {
    month <- month.name[month]
  } else if (!is.character(month) || !month %in% tolower(month.name)) {
  ...
  if(is.character(month) && tolower(month) %in% tolower(month.name)){
